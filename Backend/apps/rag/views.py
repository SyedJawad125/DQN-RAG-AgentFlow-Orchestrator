...
"""
Django Views for Multi-Agent RAG System
Complete REST API implementation — corrected version
"""
from rest_framework import status
from rest_framework.decorators import api_view
from rest_framework.response import Response
...
from apps.rag.models import (
    Document, Query, Session, AgentExecution,
    RLExperienceRecord, RLEpisodeSummary,       # ← moved to top-level
)
from apps.rag.serializers import (
...
from apps.rag.filters import DocumentFilter, QueryFilter   # ← wire filters
from apps.rag.services.core_services import (
    get_llm_service,
    get_embedding_service,
    get_vector_store,
...
# ─────────────────────────────────────────────────────────────────────────────
#  HELPER — save RL episode summary after every query
# ─────────────────────────────────────────────────────────────────────────────

def _save_rl_episode(result: dict, query_record) -> None:
    """
    Persist a per-query episode summary.
    Called at the end of query_rag(). Safe when RL metadata is absent.
    """
    rl_meta = result.get("rl_metadata", {})
    if not rl_meta:
        return
    ...
    try:
        ...
        logger.warning(f"[Views] _save_rl_episode failed: {exc}")
...
def query_rag(request):
    ...
    try:
        ...
        # ── FIX: persist RL episode — this call was previously missing ───
        _save_rl_episode(result, query_record)
...
def list_documents(request):
    ...
    Supported query params (from DocumentFilter):
        filename, status, status_in, chunks_count_min/max,
        size_min/max, uploaded_after/before, processing_time_min/max
    """
    queryset  = Document.objects.all().order_by('-uploaded_at')
    filterset = DocumentFilter(request.GET, queryset=queryset)   # ← filters wired
    documents = filterset.qs
    serializer = DocumentListSerializer(documents, many=True)
    return Response({'total': documents.count(), 'documents': serializer.data})
...
def create_session(request):
    """
    Create new session.
    POST /api/rag/v1/sessions/
    Body: { "user_id": "optional-string" }
    Returns: { "id": "uuid", "user_id": "...", "message_count": 0, ... }
    """
    user_id = request.data.get('user_id')
    session = Session.objects.create(id=uuid.uuid4(), user_id=user_id)
...
def health_check(request):
    """Health check endpoint."""
    components  = {k: 'unknown' for k in ['database', 'llm_service', 'embedding_service', 'vector_store', 'coordinator']}
    all_healthy = True

    checks = [
        ('database',          lambda: Document.objects.count()),
        ('llm_service',       get_llm_service),
        ('embedding_service', get_embedding_service),
        ('vector_store',      lambda: get_vector_store().get_count()),
        ('coordinator',       get_coordinator),
    ]
...
def list_queries(request):
    ...
    Supported query params (from QueryFilter):
        query_text, strategy_used, confidence_score_min/max,
        processing_time_min/max, created_after/before,
        document_id, session_id, agent_used
    """
    limit    = int(request.GET.get('limit', 20))
    queryset = Query.objects.all().order_by('-created_at')
    filterset = QueryFilter(request.GET, queryset=queryset)   # ← filters wired
    queries  = filterset.qs[:limit]
    return Response({
        'total':   Query.objects.count(),
        'showing': len(queries),
        'queries': QueryHistorySerializer(queries, many=True).data,
...
def get_query_execution(request, query_id):
    ...
    try:
        ...
        return Response({
            ...
            'confidence_score':query.confidence_score,
            'document_filter': query.metadata.get('document_filter'),
            'rl_metadata':     query.metadata.get('rl_metadata', {}),
        })
    except Query.DoesNotExist:
        return Response({'error': 'Query not found'}, status=status.HTTP_404_NOT_FOUND)
...
def rl_feedback(request):
    """
    Submit user feedback for a completed query.
    Triggers a deferred reward update in the Q-table.
    ...
    try:
        ...
        RLEpisodeSummary.objects.filter(query_id=query_id).update(user_feedback=feedback)
...
def rl_train(request):
    ...
    try:
        ...
        # ── FIX: delegate to the RL agent instead of reaching into internals ──
        # The RL agent exposes run_replay() which handles its own state cleanly.
        # If your agent class doesn't have run_replay() yet, add this method:
        #
        #   def run_replay(self, batch_size: int) -> dict:
        #       before = self.memory.q_table.total_updates
        #       self.memory.replay_train(batch_size=batch_size)
        #       after  = self.memory.q_table.total_updates
        #       return {
        #           "updates_before": before,
        #           "updates_after":  after,
        #           "new_updates":    after - before,
        #           "epsilon":        round(self.memory.q_table.epsilon, 4),
        #       }
        replay_result = coordinator.rl_agent.run_replay(batch_size=batch_size)
